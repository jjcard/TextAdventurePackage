...
public class DescriptionUtilTest {
	...
	public void showRoomExitsTest() {
		...
		assertFalse(roomDescrip.contains(DescriptionUtil.DEFAULT_EXIT_START), roomDescrip);
		...
		roomDescrip = DescriptionUtil.showRoom(hallway);
		assertTrue(roomDescrip.contains(DescriptionUtil.DEFAULT_EXIT_START), roomDescrip);
		assertTrue(roomDescrip.contains("NORTHISH"), roomDescrip);
		assertTrue(roomDescrip.contains("SOUTHERNLY"), roomDescrip);
		assertFalse(roomDescrip.contains("DOWN"), roomDescrip);
	...
	public void showRoomMobTest() {
		...
		assertFalse(roomDescrip
						.contains("A globin, It's chrome body shines in the torchlight, not the most stealthy then. "), roomDescrip);
		assertTrue(roomDescrip.contains("A giant stands, at least partially, in the room. It's kind of hard to miss."),
				roomDescrip);
	...
	public void showRoomItemsTest() {
		...
		assertFalse(roomDescrip.contains(item2.getRoomDescription()), roomDescrip);
		assertTrue(roomDescrip.contains(item.getRoomDescription()), roomDescrip);
	}
	@Test
	...
	public void getConcealableRoomDescriptionCollectionTest(){
		List<ConcealableGameElement> elements = null;//null check
		String result = DescriptionUtil.getConcealableRoomDescriptions(elements, true);
		assertTrue(result.isEmpty());
		result = DescriptionUtil.getConcealableRoomDescriptions(elements, false);
		assertTrue(result.isEmpty());
		
		elements = new LinkedList<>();//empty
		result = DescriptionUtil.getConcealableRoomDescriptions(elements, true);
		assertTrue(result.isEmpty());
		result = DescriptionUtil.getConcealableRoomDescriptions(elements, false);
		assertTrue(result.isEmpty());
